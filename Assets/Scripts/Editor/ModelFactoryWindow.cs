...
public class ModelFactoryWindow : EditorWindow
{
    ...
    internal static BakeConfig ConfigFor(ModelDef cur) => new BakeConfig
    {
        ...
        deployRecoil = (cur.deployRecoil ?? "").Trim(), deployRecoilStep = (cur.deployRecoilStep ?? "").Trim(), deployRecoilMag = (cur.deployRecoilMag ?? "").Trim(), deployArcR = (cur.deployArcR ?? "").Trim(), deployRecoilReturn = (cur.deployRecoilReturn ?? "").Trim(),
    ...
    void DoBake()
    {
        ...
        // shipping a 100×-giant soldier). This window contributes everything else (model file, transform, size, …).
        {
            ...
            if (regE != null)
            {
                ...
                cur.deployRecoil = regE.deployRecoil; cur.deployRecoilStep = regE.deployRecoilStep; cur.deployRecoilMag = regE.deployRecoilMag; cur.deployArcR = regE.deployArcR; cur.deployRecoilReturn = regE.deployRecoilReturn;
