...
public class ModelDef
{
    ...
    public float animPhaseSpread = 0f;  // RUNTIME (not baked): spread the LOOPING animation across a unit's pawns so they don't all move in lockstep (12 canoes rocking as one rigid raft). 0 = lockstep (old behaviour), 1 = instances spread over the whole clip, 0.5 = over half of it. Each pawn is offset deterministically by its slot index, so the look is stable frame to frame. One-shots (attack/deploy/fire) are timed from their trigger and never shifted.
