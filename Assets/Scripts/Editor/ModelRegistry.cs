...
public class ModelDef
{
    ...
    public bool autoGroundWheels = false; // ANIMATED only (BAKE-TIME): sit a rigged VEHICLE on the terrain automatically — the bake measures how far the ROTATING (wheel) bones protrude below the static body (minZ(body) − minZ(all)) and lifts the rig by it, so you never dial the Position offset by hand. Opt-in: a full-body rig (mech/soldier) or deploy rig (howitzer legs) would mis-detect the moving parts.
