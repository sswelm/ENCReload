...
public class ModelDef
{
    ...
    public float gunElevMax;            // RUNTIME-ONLY (GUN ELEVATION): max barrel raise (deg) during a bombard, PROPORTIONAL to target distance (full at ~3 tiles) — rises while the hull turns, holds through the shot, lowers after. Applies to turretBone else muzzleBone. 0 = off; negative flips. Save (no bake) + relaunch.
    public int gunElevAxis;             // RUNTIME-ONLY: the gun bone's local PITCH axis for the elevation (0=X usual, 1=Y, 2=Z).
