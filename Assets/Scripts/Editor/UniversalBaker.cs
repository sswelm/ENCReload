...
public struct BakeConfig
{
    ...
    public bool    autoGroundWheels;// ANIMATED only: auto-sit a rigged VEHICLE on the terrain — measure how far the ROTATING (wheel) bones protrude below the static body and lift the rig by it (no manual Position-offset dial). Opt-in: a full-body or deploy rig would mis-detect.
...
public static class UniversalBaker
{
    ...
    static BakeResult BuildAnimatedInner(BakeConfig cfg)
    {
        ...
        if (!string.IsNullOrEmpty(cfg.modelFile) && (!cfg.reuseExtracted || !File.Exists(fbxFull) || roleFbxMissing || toolNewer || srcNewer))
        {
            ...
            if (!RigAnimViaBlender(cfg.modelFile, fbxFull, target, cfg.animateBones ?? "", cfg.animClip ?? "", albedoOut, keepMats, cfg.rotationEuler, cfg.convertRig, stateRoles, cfg.autoGroundWheels))
    ...
    static bool RigAnimViaBlender(string src, string outFbx, int targetTris, string bonePrefixes, string clipName, string albedoOut, bool keepMaterials, Vector3 rotation, bool convertRig, string stateRoles = "", bool autoGround = false)
    {
        ...
        string args = $"--background --python \"{script}\" -- \"{src}\" \"{outFbx}\" {Mathf.Max(1, targetTris)} \"{bonePrefixes ?? ""}\" \"{clipName ?? ""}\" \"{albedoOut ?? ""}\" {(keepMaterials ? "1" : "0")} \"{rotArg}\" {(convertRig ? "1" : "0")} \"{stateRoles ?? ""}\" {(autoGround ? "1" : "0")}";   // argv[10]: auto-ground wheeled vehicles
    ...
    const int AtlasMaxDimDefault = 512;
